...
ava('aliases, noop parser and zeroArgumentsValidator used by contactTower', t => {
    const [parse, validate] = extractParseAndValidate(t, 'contactTower');
    t.true(parse === noopParse() && validate === zeroArgVal());
    test_aliases(t, AIRCRAFT_COMMAND_MAP, 'contactTower', ['ct', 'contacttower']);
});

ava('aliases, noop parser and zeroArgumentsValidator used by contactCenter', t => {
    const [parse, validate] = extractParseAndValidate(t, 'contactCenter');
    t.true(parse === noopParse() && validate === zeroArgVal());
    test_aliases(t, AIRCRAFT_COMMAND_MAP, 'contactCenter', ['cc', 'contactcenter']);
});
...
ava('make sure we test all 34 aircraft commands', t => {
    t.true(Object.values(AIRCRAFT_COMMAND_MAP)
        .filter(val => !val.isSystemCommand).length === 34);
});
