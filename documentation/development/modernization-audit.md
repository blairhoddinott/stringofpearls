...
Phase 4 now has a browser-free `SimulationContext` with context-owned event dispatch, deterministic time, game timers, airport-controller state, navigation/fix state, traffic patterns, traffic scheduling, and aircraft collection state. Each context creates fresh clock, event-bus, `SimulationTimerQueue`, `AirportControllerClass`, `NavigationLibraryClass`, `FixCollectionClass`, `SpawnPatternCollectionClass`, `SpawnSchedulerClass`, and `AircraftCollection` instances by default; `tick(delta)` advances that clock, processes only that session's timers, and then updates only that session's aircraft controller, preserving the inherited timer-before-aircraft order. `destroy()` clears only that session's observers, timers, aircraft membership, airport registry/current selection, navigation state, and traffic patterns. Explicitly injected clock, event, airport, navigation, aircraft-collection, aircraft-controller, and spawn-pattern services are retained by exact identity. When an aircraft controller is injected without repeating its service owners, the context adopts that controller's clock, event bus, airport controller, navigation library, and collection; explicitly mismatched owner pairs fail construction. Context-created navigation and traffic services receive the context's exact random-source identity, while context-created airport and traffic services retain the session's event, airport, navigation, clock, and timer capabilities.
