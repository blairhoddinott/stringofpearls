...
Phase 4 now has a browser-free `SimulationContext` with context-owned event dispatch, deterministic time, and game timers. Each context creates fresh clock, event-bus, and `SimulationTimerQueue` instances by default; `tick(delta)` advances that clock before processing only that session's timers, and `destroy()` clears only that session's observers and timers. `SimulationClock` retains the explicit pause, non-negative timewarp, 100-second cap, and elapsed-time policy established in the previous checkpoint while exposing the legacy `accumulatedDeltaTime` read contract needed for incremental consumer migration. The timer queue preserves the inherited six-slot timer arrays, strict `currentTime > deadline` firing rule, callback receiver/data shape, interval rescheduling from the prior deadline, selected-timer removal behavior, and fresh-array replacement on bulk destruction. `GameControllerClass` is now constructible with an exact timer-queue identity, while the unchanged default singleton delegates its public timer methods to a queue backed by legacy `TimeKeeper`; this migrates a real production consumer without changing the App frame loop or unmigrated physics consumers. The core browser-free proof executes isolated event, clock, and timer behavior in two contexts. Acceptance reports 1,597 passing, 17 skipped, and 14 todo tests; all 131 eligible source files appear in coverage, lines/statements are 72.91%, functions are 71.11%, branches are 93.09%, and lint, the deterministic 278-file build, and KPDX browser smoke pass. The next Phase 4 slice will move airport/navigation state behind the per-session context.
