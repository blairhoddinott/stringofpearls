...
- [ ] Modernize the application architecture
  - [ ] Introduce explicit platform boundaries; asset loading uses injected native-Promise adapters throughout, the storage front is complete, and wall-clock acquisition now uses a shared injected clock adapter, while animation-frame/delayed-callback scheduling, randomness, analytics, speech synthesis, clipboard/page visibility, and the final browser-free proof still require migration.
