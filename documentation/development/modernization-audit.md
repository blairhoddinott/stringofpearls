...
Phase 4 now has a browser-free `SimulationContext` with context-owned event dispatch, deterministic time, game timers, and navigation/fix state. Each context creates fresh clock, event-bus, `SimulationTimerQueue`, `NavigationLibraryClass`, and `FixCollectionClass` instances by default; `tick(delta)` advances that clock before processing only that session's timers, and `destroy()` clears only that session's observers, timers, and navigation state. A context-created navigation library receives the context's exact random-source identity for procedure exit selection, while an explicitly injected navigation library is retained unchanged.

The navigation checkpoint retains the legacy default `NavigationLibrary` and `FixCollection` singleton exports for unmigrated production consumers while exposing constructible named classes. Airway validation now uses the owning library passed by `NavigationLibraryClass`, removing the inherited `NavigationLibrary`/`AirwayModel` import cycle. Airway- and procedure-generated `WaypointModel` instances resolve positions through that same library's fix collection rather than the global singleton. Focused tests and the fresh-process browser-free proof exercise two independent navigation graphs, airway expansion, context-local destruction, and random-source propagation. Production `App` composition and `AirportController` still use the legacy singleton graph and remain explicit work for the next navigation slice.

Acceptance reports 1,603 passing, 17 skipped, and 14 todo tests; all 131 eligible source files appear in coverage, lines/statements are 72.87%, functions are 71.19%, branches are 92.99%, and lint, the deterministic 278-file build, KPDX browser smoke, the browser-free proof, and the production audit pass. The next Phase 4 slice will move airport-controller state behind the per-session context without changing the current browser frame loop.
