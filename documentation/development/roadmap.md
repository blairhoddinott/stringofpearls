...
- [ ] Modernize the application architecture
  - [ ] Introduce explicit platform boundaries; asset loading uses injected native-Promise adapters throughout, and startup selection, airport persistence, changelog versioning, tutorial completion, canvas zoom, and game options use the shared storage adapter while the remaining storage consumers and browser services still require migration.
