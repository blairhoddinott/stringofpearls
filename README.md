...
## Pilot speech requirements

Pilot responses use the browser's [Web Speech synthesis API](https://developer.mozilla.org/en-US/docs/Web/API/SpeechSynthesis). Speech is generated by the browser on the user's computer, not by the application container, so rebuilding the Docker image does not install voices for the browser.

Windows and macOS browsers normally use voices supplied by the operating system. Firefox on Linux requires a working [Speech Dispatcher](https://wiki.archlinux.org/title/Speech_dispatcher) installation and at least one speech-synthesis engine. On Arch Linux:

```sh
sudo pacman -S speech-dispatcher espeak-ng
systemctl --user enable --now speech-dispatcher.socket
spd-say "String of Pearls speech test"
```

Fully restart Firefox after installing or restarting Speech Dispatcher. Verify that Firefox can see at least one voice from its developer console:

```js
speechSynthesis.getVoices().map(({ name, lang }) => ({ name, lang }))
```
