...
The aircraft-update checkpoint gives `SimulationClock` the inherited effective-delta reader expected by aircraft physics and threads the exact clock and event bus through `AircraftController` into every created `AircraftModel`. Aircraft elapsed-time, turn, altitude, speed, position, taxi, takeoff, and hold calculations now use the owning clock; takeoff, approach, final-approach, airspace-exit, and full-stop events use the owning event bus. Conflicts created by an owner-bound controller retain that same clock and event bus plus the context airport controller, so conflict timing, airport elevation, collision airspace, and runway-relationship calculations no longer return to compatibility singletons. `SimulationContext.tick(delta)` preserves the established clock-then-timers-then-aircraft ordering, leaves strip/render updates outside the domain boundary, updates no sibling controller, and returns the clock result verbatim. The fresh-process browser-free proof exercises repeated aircraft updates in one context while the sibling remains untouched. Conflict duplicate detection and UI logging still use inherited presentation globals.

The game-state checkpoint extracts inherited event names, point values, and descriptions into a dependency-free constants module while preserving `GameController`'s public `GAME_EVENTS` export. Each context owns a browser-free `SimulationGameState` with isolated score, event counts, and tower-controller mode. Recording validates the inherited event set and applies the exact inherited point values; teardown restores zero score/counts and the instance's configured initial tower mode. `AircraftController`, `AircraftModel`, and `AircraftConflict` receive that exact state owner, and owner-bound aircraft no longer call `GameController.events_recordNew()`, `getGameOption()`, or `game_speedup()`. `ScoreController` accepts explicit event-bus and game-state owners so context composition can isolate approach, landing, wind, runway-separation, and airspace-exit scoring while retaining its legacy singleton defaults. It remains externally composed because user-facing score warnings still call `UiController`; no presentation controller was pulled into `SimulationContext`. The fresh-process proof exercises score/option isolation and local teardown without browser globals.

Acceptance reports 1,659 passing, 17 skipped, and 14 todo tests; all 134 eligible source files appear in coverage, lines/statements are 74.71%, functions are 73.32%, branches are 93.05%, and lint, the deterministic 278-file build, KPDX browser smoke with zero uncaught errors, the browser-free proof, and the production audit pass. The browser frame loop remains unchanged.
