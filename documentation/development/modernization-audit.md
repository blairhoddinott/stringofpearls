...
Phase 4 now has a browser-free `SimulationContext` shell and a context-owned deterministic `SimulationClock`. Each context creates fresh clock and event-bus instances by default, may retain exact injected clock, random-source, and event-bus identities, and destroys only its own event observers. `SimulationContext.tick(delta)` forwards the exact delta once and returns the clock result verbatim. The default clock starts active at rate one, scales explicit deltas by the configured non-negative simulation rate, accepts zero as a stopped rate, caps effective deltas at the inherited 100-second maximum, freezes both effective delta and elapsed time while paused, and resumes using only the next explicit delta rather than a wall-clock catch-up heuristic. Two contexts therefore advance independently without current-time reads. The legacy `EventBus` and `TimeKeeper` singleton exports, App update loop, and downstream time consumers remain unchanged; this checkpoint establishes deterministic policy but does not claim consumer migration. Acceptance reports 1,590 passing, 17 skipped, and 14 todo tests; all 130 eligible source files appear in coverage, lines/statements are 72.82%, functions are 70.93%, branches are 93.05%, and lint, the core browser-free proof, deterministic 278-file build, and KPDX browser smoke pass. The next time tracer will redirect one real simulation-time consumer through the context boundary before widening the migration.
