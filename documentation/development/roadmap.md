...
- [ ] Modernize the application architecture; Phase 4 now has a browser-free per-session `SimulationContext`, isolated event dispatch, deterministic time, context-owned game timers, isolated airport-controller/navigation state, context-owned traffic patterns/scheduling, isolated aircraft collections, and owner-bound scheduled aircraft construction.
