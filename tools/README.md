...
The current Phase 5 baseline reports 1,836 passing, 17 skipped, and 14 todo tests. [c8][c8] provides Node 24 coverage:
...
The generated application and same-origin backend APIs are served by the repository-owned Express service in development and production. `npm run server:test` first runs the focused weather contracts, builds the application, and then verifies health, security and cache headers, API routing, root/static-asset responses, SPA fallback, encoded traversal rejection, and clean server shutdown. The hardened runtime is described in [`documentation/development/containers.md`](../documentation/development/containers.md).
