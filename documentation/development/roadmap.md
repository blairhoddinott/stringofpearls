...
- [ ] Modernize the application architecture
  - [ ] Introduce explicit platform boundaries; asset loading now uses injected native-Promise adapters throughout and startup airport selection uses an injected storage adapter, while the remaining storage consumers and browser services still require migration.
